...
**F-35 内容优化任务骨架（T02）**：行为：基准简历行「内容优化」入口 → ContentOptimizeService 异步任务（状态机 `created→diagnosing→awaiting_answers→rewriting→ready_for_review→confirmed`；failed 手动重试、cancelled 续接或作废、应用重启自动恢复中断轮次）；LLM 轮次全局串行队列（每轮超时 180s、重试 1 次）；任务记录存 `content_optimize_tasks` 独立存储；首次触发自动补齐项目稳定 ID/≤4 条要点/sectionOrder（#91）；空诊断（全部保持）→「无需修改」不创建新版本；事件 `content-optimize:changed` 实时推送阶段流转。E2E 基建：`JOBHUNT_FAKE_AGENT=1` 假 agent + 真实启动 Electron + CDP 鼠标/键盘驱动 + 截图/无障碍快照断言 + 落库断言。规则：#90-27（只对基准简历开放）、#90-21（确认后成为新基准简历）。验收：状态机/重试/取消续接/单基准单草稿/持久化单测绿；E2E 冒烟（`npm run e2e:content-optimize`）通过。
...
**F-38 大赛经历提升为项目（T08）**：行为：诊断引擎识别 honors 中大赛/竞赛条目并输出提升建议（`promotions`，含稳定 id/下标/honorName/证据/缺失字段）；任务卡展示提升建议区（需用户确认，可「暂不提升」）；缺失字段（开始/结束时间/技术栈/描述）经追问表单补齐（四选一+自由输入，哨兵不写字段）；`confirmPromotion` 应用提升：honors 移除该条目 → projects 追加新项目（稳定 id，字段来自回答），回答并入任务 answers，随后回到 `diagnosing` 重新诊断（awaiting_answers→diagnosing 循环）——提升后的项目以真实项目 id 参与规则诊断与改写（#90 边界：大赛经历归属项目经历）。校验：仅 awaiting_answers 可确认；回答键限缺失字段、值非空；honors 下标与 honorName 双重匹配（诊断后简历被改则拒绝）；解析层丢弃空 honorName 建议。migration 无新增列（提升结果落简历本身）。验收：诊断产出建议/确认提升/缺失字段补齐/提升后重诊断参与改写单测绿；E2E promotion 场景（`npm run e2e:content-optimize:promotion`）含提升区无障碍断言与落库断言（honors 移除/项目追加）通过。

**F-39 内容优化全流程 E2E 冒烟（T09）**：行为：基于 T02 E2E 基建扩展为全流程冒烟——真实启动 Electron（假 agent 默认确定性运行；`JOBHUNT_E2E_REAL=1` 时可选真实 agent 冒烟入口），以 CDP 鼠标/键盘驱动走通「触发内容优化 → 诊断进度 → 追问作答 → 逐项确认（接受改写 + 推断-待确认勾选）→ 入库新基准」；断言为截图/无障碍快照 + 落库状态（`content_optimize_tasks` 任务存储与 `resumes` 表）双重验证。full 场景（`npm run e2e:content-optimize:full`）断言：诊断中进度可见、追问表单作答（已答 X/Y）、逐项目确认区（接受/保留/来源）、推断-待确认门禁（先勾选后确认按钮恢复）、确认后任务 confirmed + 新基准入库（旧基准保留、血缘 `created_resume_id`/`archived_at` 落任务）。真实 agent 冒烟（`npm run e2e:content-optimize:real`）需 `JOBHUNT_E2E_USER_DATA` 指向已配置 agent 的持久 userData 目录（见 `scripts/e2e-content-optimize-real.md`），脚本仅驱动「触发 → 到达稳定阶段」，失败以非零码退出并给出错误说明；持久目录绝不被脚本清理。定位为冒烟接缝（依赖真实显示环境），非全量回归。验收：full 场景落库断言（任务/answers/diagnosis/rewrite/inferred/血缘）与 UI 一致；四个场景（empty/questions/promotion/full）均通过；真实 agent 入口文档可用。
